...
**16.1% of the workforce left (237 of 1,470) — ~$3.6M/year at a conservative $15K per leaver — and the loss concentrates in four places: overtime workers (30.5% vs 10.4% attrition, ~6× adjusted odds), first-year employees (34.9%), the bottom pay quartile (29.3% vs 10.3%), and the Sales Representative role (39.8%).** A logistic-regression risk model puts 49% of actual leavers inside a watch list covering just 10% of employees (held-out AUC 0.865, top-decile lift 4.9×). Full story: [memo.md](memo.md).

Also notable: the "fancier" model lost. XGBoost underperformed logistic regression on the same held-out test set (AUC 0.825 vs 0.865) — the honest small-tabular-data result, written up in [05-model-comparison.md](05-model-comparison.md). XGBoost still earns its keep via SHAP: every employee on the watch list carries a named top risk driver.
...
│   ├── attrition-risk-scores.csv  # per-employee risk score, decile, top SHAP driver
...
- **Predictive modeling in two stacks** for an honest comparison on one shared held-out test set (stratified 80/20, seed 42, committed to the repo). Logistic regression in R for interpretable odds ratios with CIs. XGBoost in Python (class-weighted, early stopping) with SHAP for per-employee attribution.
